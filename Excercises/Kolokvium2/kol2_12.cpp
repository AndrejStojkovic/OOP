...
class Device {
    char model[100];
    tip type; // 0 - smartphone, 1 - computer
    static float hours;
    int year;
...
public:
    ...
    friend ostream& operator<<(ostream& out, Device& device) {
        out << device.model << "\n";
        out << Tip[device.type] << " " << device.proverka() << "\n";
        return out;
    }

    static void setPocetniCasovi(float _hours) { Device::hours = _hours; }

    float proverka() {
        float s = 0;

        if(year > 2015) s += 2;
        if(type == computer) s+= 2;
    ...
    void setType(tip type) { this->type = type; }
...
public:
    ...
    MobileServis& operator+=(Device& device) {
        if(device.getYear() < 2000 || device.getYear() > 2019) throw InvalidProductionDate();

        Device* tmp = new Device[n + 1];
        for(int i = 0; i < n; i++) tmp[i] = this->d[i];
        tmp[n++] = device;

        delete [] d;
        this->d = new Device[n];
        for(int i = 0; i < n; i++) this->d[i] = tmp[i];
        delete [] tmp;
